...
import SquarePyramidStack from './SquarePyramidStack.vue';
...
export interface ISquarePyramidProps {
  /**
   * Number of square layers in the pyramid (1 to 6).
   * Automatically inferred from slotted `<SquarePyramidStack>` components if omitted.
   * Default: 4
   */
  readonly count?: number;
  /**
   * Index of the active square layer (0 = top, count - 1 = bottom).
   ...
   * If omitted, distinct defaults from DEFAULT_SQUARE_PYRAMID_STACK_COLORS are used.
   */
...
const props = withDefaults(defineProps<ISquarePyramidProps>(), {
  count: undefined,
  activeIndex: -1,
  color: DEFAULT_SQUARE_PYRAMID_COLOR,
  colors: undefined,
  width: undefined,
  height: undefined,
  scale: undefined,
...
function countSlottedStacks(slotNodes: VNode[] | undefined): number {
  ...
  for (const node of slotNodes) {
    if (typeof node.type === 'object' && node.type !== null) {
      ...
      if (
        comp?.name === 'SquarePyramidStack' ||
        comp?.displayName === 'SquarePyramidStack'
      ) {
        count++;
        continue;
      }
...
function getLayerColor(index: number): string {
  ...
  const stackColor =
    DEFAULT_SQUARE_PYRAMID_STACK_COLORS[
      Math.abs(index) % DEFAULT_SQUARE_PYRAMID_STACK_COLORS.length
    ];

  return stackColor ?? props.color ?? DEFAULT_SQUARE_PYRAMID_COLOR;
}
...
const resolvedCount = computed(() => {
  if (props.count !== undefined) {
    return clampSquareCount(props.count);
  }
  if (slottedStackCount.value > 0) {
    return clampSquareCount(slottedStackCount.value);
  }
  if (registeredStackTokens.value.length > 0) {
    return clampSquareCount(registeredStackTokens.value.length);
  }
  return 4;
});
...
const geo = computed(() => {
  ...
  return createSquarePyramidGeometry({
    ...
    slabHeight: resolvedCount.value >= 5 ? 8 : 10,
    ratio: 0.46,
  });
});

const palette = computed(() => {
  return resolveSquarePyramidPalette(props.color);
});
...
provide(SQUARE_PYRAMID_ROOT_KEY, {
  count: resolvedCount,
...
const filterActiveShadowId = `alpha-square-pyramid-shadow-${uid}`;
const filterAmbientShadowId = `alpha-square-pyramid-ambient-${uid}`;

const activeTopGradId = `alpha-sq-grad-act-top-${uid}`;
const activeSideGradId = `alpha-sq-grad-act-side-${uid}`;
const defaultTopGradId = `alpha-sq-grad-def-top-${uid}`;
const defaultSideGradId = `alpha-sq-grad-def-side-${uid}`;

function getInactiveTopGradId(index: number): string {
  return `alpha-sq-grad-inact-top-${uid}-${index}`;
}

function getInactiveSideGradId(index: number): string {
  return `alpha-sq-grad-inact-side-${uid}-${index}`;
}

function getActiveTopGradId(index: number): string {
  return `alpha-sq-grad-act-top-${uid}-${index}`;
}

function getActiveSideGradId(index: number): string {
  return `alpha-sq-grad-act-side-${uid}-${index}`;
}

function getActiveShadowId(index: number): string {
  return `alpha-sq-shadow-${uid}-${index}`;
}

const { resolvedScale } = useDiagramAutoScale({
  rootRef,
...
const { className, forwardedAttrs } = useMergedUnoAttrs(
  'alpha-square-pyramid w-full flex items-center justify-center select-none relative',
);
</script>
...
<template>
  <div
    ...
    :style="{
      ...
      '--square-pyramid-start-delay': `${resolvedStartDelay}ms`,
      '--square-pyramid-title-width': `${titleWidthPx}px`,
      '--square-pyramid-content-width': `${contentWidthPx}px`,
    }"
    @click="handleContainerClick"
  >
    <div
      class="alpha-square-pyramid__stage relative flex items-center justify-center overflow-visible flex-shrink-0"
      :style="{
    ...
    >
      <!-- Central Square Pyramid SVG Graphic -->
      <div
        class="alpha-square-pyramid__middle absolute inset-y-0 flex items-center justify-center pointer-events-none overflow-visible"
        style="left: 150px; right: 180px;"
      >
        <svg
          class="alpha-square-pyramid__svg w-full h-full overflow-visible pointer-events-none"
        ...
        >
          <defs>
            ...
            <template v-for="layer in geo.layers" :key="layer.index">
              <!-- Inactive Frosted Glass Top Face Gradient -->
              <linearGradient
              ...
              <!-- Inactive Frosted Glass Rim Gradient -->
              <linearGradient
          ...
          <!-- Ambient Ground Shadow (under the lowest pyramid slab) -->
          <ellipse
            v-if="lowestLayer"
            :cx="geo.viewBoxWidth / 2"
            :cy="lowestLayer.cy + lowestLayer.slabHeight + 14"
            :rx="lowestLayer.rx * 0.92"
            :ry="lowestLayer.ry * 0.3"
            fill="#0f172a"
            opacity="0.1"
            :filter="`url(#${filterAmbientShadowId})`"
          />
          <!-- Stacked Floating Square Slabs (Rendered from bottom to top) -->
          <g
            class="alpha-square-pyramid__stack"
            :class="{ 'alpha-square-pyramid--animated': isAnimated }"
          >
            <g
              v-for="layer in stackedLayers"
              :key="layer.index"
              class="alpha-square-pyramid__layer transition-all duration-300 ease-out pointer-events-auto"
              :class="[
                layer.index === currentActiveIndex
                  ? 'alpha-square-pyramid__layer--active'
                  : 'alpha-square-pyramid__layer--default',
                `alpha-square-pyramid__layer--${layer.index}`,
            ...
            >
              ...
              <path
                class="alpha-square-pyramid__rim transition-all duration-300"
                :d="layer.sidePath"
                :fill="
                  layer.index === currentActiveIndex
                ...
                "
              />
              <!-- 2. Slab Top Face Isometric Rounded Diamond -->
              <path
                class="alpha-square-pyramid__face transition-all duration-300"
                :d="layer.topFacePath"
                :fill="
                  layer.index === currentActiveIndex
                ...
                stroke-width="0.5"
              />
              <!-- 3. Inner Inset Highlight (matches translucent glassmorphic plate) -->
              <path
                class="alpha-square-pyramid__inner transition-all duration-300 pointer-events-none"
                :d="layer.innerFacePath"
                fill="#ffffff"
                :fill-opacity="layer.index === currentActiveIndex ? 0.35 : 0.2"
              />
            </g>
          </g>
        </svg>
...
<style scoped>
.alpha-square-pyramid {
  box-sizing: border-box;
}

.alpha-square-pyramid__layer {
  transform-box: fill-box;
  transform-origin: center;
}

.alpha-square-pyramid__layer:hover {
