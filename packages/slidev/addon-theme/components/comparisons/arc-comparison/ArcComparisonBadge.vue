...
export interface IArcComparisonBadgeProps {
  readonly id?: string | number;
  readonly color?: string;
  readonly size?: number;
}

const props = withDefaults(defineProps<IArcComparisonBadgeProps>(), {
  id: undefined,
  color: undefined,
  size: 52,
});
...
const { className, forwardedAttrs } = useMergedUnoAttrs(
  'alpha-arc-comparison-badge rounded-full flex items-center justify-center font-bold text-white shadow-md select-none flex-shrink-0 transition-transform duration-300',
);
</script>
